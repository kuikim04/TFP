        ...
        public Color[] GetPixels()
        {
            ...
            if (Controls)
            {
                ...
                for (var i = 0; i < pixels.Length; i++)
                {
                    ...
                    if (PaintMode == PaintMode.IgnoreGray)
                    {
                        if (Math.Abs(pixels[i].r - pixels[i].g) < 0.1 && Math.Abs(pixels[i].r - pixels[i].b) < 0.1) continue;

                        Color.RGBToHSV(pixels[i], out _, out _, out var value);

                        pixels[i] = new Color(Color.r * value, Color.g * value, Color.b * value, Color.a);
