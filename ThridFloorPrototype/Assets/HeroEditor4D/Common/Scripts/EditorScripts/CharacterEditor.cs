...
namespace Assets.HeroEditor4D.Common.Scripts.EditorScripts
{
    ...
    public class CharacterEditor : MonoBehaviour
    {
        ...
        public void LoadFromSave(string json)
        {
            try
            {
                var characterData = JsonConvert.DeserializeObject<Character4D>(json);
                if (characterData != null)
                {
                    Character.CopyFrom(characterData);
                    Debug.Log("Character loaded successfully from save.");
                }
                else
                {
                    Debug.LogError("Failed to load character: Deserialized data is null.");
                }
            }
            catch (Exception ex)
            {
                Debug.LogError($"Failed to load character from save: {ex.Message}");
            }
        }
    }
}